...
#ifdef HAS_DS_PLAYER
                           || CServiceBroker::GetGUI()->GetWindowManager().IsWindowActive(WINDOW_DIALOG_LAVVIDEO)
                           || CServiceBroker::GetGUI()->GetWindowManager().IsWindowActive(WINDOW_DIALOG_LAVAUDIO)
                           || CServiceBroker::GetGUI()->GetWindowManager().IsWindowActive(WINDOW_DIALOG_LAVSPLITTER)
                           || CServiceBroker::GetGUI()->GetWindowManager().IsWindowActive(WINDOW_DIALOG_SANEAR)
#endif
  ...
  case GUI_MSG_WINDOW_DEINIT:  // fired when OSD is hidden
    {
      ...
      if (pDialog && pDialog->IsDialogRunning())
        pDialog->Close(true);
#ifdef HAS_DS_PLAYER
      pDialog = (CGUIDialog *)CServiceBroker::GetGUI()->GetWindowManager().GetWindow(WINDOW_DIALOG_VIDEO_OSD_SETTINGS);
      if (pDialog && pDialog->IsDialogRunning()) 
        pDialog->Close(true);
      pDialog = (CGUIDialog *)CServiceBroker::GetGUI()->GetWindowManager().GetWindow(WINDOW_DIALOG_LAVVIDEO);
      if (pDialog && pDialog->IsDialogRunning()) 
        pDialog->Close(true);
      pDialog = (CGUIDialog *)CServiceBroker::GetGUI()->GetWindowManager().GetWindow(WINDOW_DIALOG_LAVAUDIO);
      if (pDialog && pDialog->IsDialogRunning()) 
        pDialog->Close(true);
      pDialog = (CGUIDialog *)CServiceBroker::GetGUI()->GetWindowManager().GetWindow(WINDOW_DIALOG_LAVSPLITTER);
      if (pDialog && pDialog->IsDialogRunning()) 
        pDialog->Close(true);
      pDialog = (CGUIDialog *)CServiceBroker::GetGUI()->GetWindowManager().GetWindow(WINDOW_DIALOG_SANEAR);
      if (pDialog && pDialog->IsDialogRunning())
        pDialog->Close(true);
#endif
    }

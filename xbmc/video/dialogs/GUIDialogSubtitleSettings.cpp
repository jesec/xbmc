...
#ifdef HAS_DS_PLAYER
void CGUIDialogSubtitleSettings::ShowSubsSelector()
{
  int count = g_application.GetAppPlayer().GetSubtitleCount();

  if (count <= 0)
  {
    CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Info, g_localizeStrings.Get(462),
                                          g_localizeStrings.Get(55059), 2000, false, 300);
    return;
  }

  CGUIDialogSelect* pDlg =
      (CGUIDialogSelect*)CServiceBroker::GetGUI()->GetWindowManager().GetWindow(
  ...
  pDlg->EnableButton(true,
                     g_application.GetAppPlayer().GetVideoSettings().m_SubtitleOn ? 55058 : 13397);
  pDlg->Open();
  ...
  {
    g_application.GetAppPlayer().SetSubtitle(
        selected); // Set the subtitle stream to the one selected
  }

...
bool CDSMediaPortalCards::ParseLines(vector<string>& lines)
{
  ...
  for (vector<string>::iterator it = lines.begin(); it < lines.end(); ++it)
  {
    ...
    if (!data.empty())
    {
      ...
      card.GrabEPG = (CStdStringA(fields[4]).MakeLower().compare("false") == 0) ? false : true;
      card.LastEpgGrab.SetFromDateString(fields[5]);
      card.RecordingFolder = fields[6];
      card.IdServer = atoi(fields[7].c_str());
      card.Enabled = (CStdStringA(fields[8]).MakeLower().compare("false") == 0) ? false : true;
      ...
      card.Preload = (CStdStringA(fields[13]).MakeLower().compare("false") == 0) ? false : true;
      card.CAM = (CStdStringA(fields[14]).MakeLower().compare("false") == 0) ? false : true;
      card.NetProvider = atoi(fields[15].c_str());
      card.StopGraph = (CStdStringA(fields[16]).MakeLower().compare("false") == 0) ? false : true;

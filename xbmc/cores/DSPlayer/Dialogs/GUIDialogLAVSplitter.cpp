...
void CGUIDialogLAVSplitter::InitializeSettings()
{
  ...
  const std::shared_ptr<CSettingCategory> category = AddCategory("dsplayerlavsplitter", -1);
  if (category == NULL)
  {
  ...
  const std::shared_ptr<CSettingGroup> groupPreflang = AddGroup(category);
  ...
  // get all necessary setting groups
  const std::shared_ptr<CSettingGroup> groupSubmode = AddGroup(category);
  ...
  // get all necessary setting groups
  const std::shared_ptr<CSettingGroup> groupBluraysub = AddGroup(category);
  ...
  // get all necessary setting groups
  const std::shared_ptr<CSettingGroup> groupFormat = AddGroup(category);
  if (groupFormat == NULL)
  {
    CLog::Log(LOGERROR, "CGUIDialogLAVSplitter: unable to setup settings");
    return;
  }
  const std::shared_ptr<CSettingGroup> groupDemuxer = AddGroup(category);
  if (groupDemuxer == NULL)
  {
    CLog::Log(LOGERROR, "CGUIDialogLAVSplitter: unable to setup settings");
    return;
  }
  const std::shared_ptr<CSettingGroup> groupQueueNet = AddGroup(category);
...
void CGUIDialogLAVSplitter::OnSettingChanged(std::shared_ptr<const CSetting> setting)
{
  if (setting == NULL)
    return;
  ...
  if (settingId == LAVSPLITTER_TRAYICON)
    lavSettings.splitter_bTrayIcon = static_cast<BOOL>(static_pointer_cast<const CSettingBool>(setting)->GetValue());
  if (settingId == LAVSPLITTER_PREFAUDIOLANG)
  { 
    g_charsetConverter.utf8ToW(static_cast<std::string>(static_pointer_cast<const CSettingString>(setting)->GetValue()), strW, false);
  ...
  if (settingId == LAVSPLITTER_SUBMODE)
    lavSettings.splitter_subtitleMode = (LAVSubtitleMode)static_cast<int>(static_pointer_cast<const CSettingInt>(setting)->GetValue());
  if (settingId == LAVSPLITTER_PGSFORCEDSTREAM)
    lavSettings.splitter_bPGSForcedStream = static_cast<BOOL>(static_pointer_cast<const CSettingBool>(setting)->GetValue());
  if (settingId == LAVSPLITTER_PGSONLYFORCED)
    lavSettings.splitter_bPGSOnlyForced = static_cast<BOOL>(static_pointer_cast<const CSettingBool>(setting)->GetValue());
  if (settingId == LAVSPLITTER_IVC1MODE)
    lavSettings.splitter_iVC1Mode = static_cast<BOOL>(static_pointer_cast<const CSettingBool>(setting)->GetValue());
  if (settingId == LAVSPLITTER_MATROSKAEXTERNAL)
    lavSettings.splitter_bMatroskaExternalSegments = static_cast<BOOL>(static_pointer_cast<const CSettingBool>(setting)->GetValue());
  if (settingId == LAVSPLITTER_SUBSTREAM)
    lavSettings.splitter_bSubstreams = static_cast<BOOL>(static_pointer_cast<const CSettingBool>(setting)->GetValue());
  if (settingId == LAVSPLITTER_REMAUDIOSTREAM)
    lavSettings.splitter_bStreamSwitchRemoveAudio = static_cast<BOOL>(static_pointer_cast<const CSettingBool>(setting)->GetValue());
  if (settingId == LAVSPLITTER_PREFHQAUDIO)
    lavSettings.splitter_bPreferHighQualityAudio = static_cast<BOOL>(static_pointer_cast<const CSettingBool>(setting)->GetValue());
  if (settingId == LAVSPLITTER_IMPAIREDAUDIO)
    lavSettings.splitter_bImpairedAudio = static_cast<BOOL>(static_pointer_cast<const CSettingBool>(setting)->GetValue());
...
void CGUIDialogLAVSplitter::OnSettingAction(std::shared_ptr<const CSetting> setting)
{
  if (setting == NULL)
    return;
